...
class employeeClass:
    def __init__(self, root):
        ...
        cmb_gender = ttk.Combobox(self.root, textvariable=self.var_gender, values=("Select", "Male", "Female", "Other"), state="readonly", justify="center", font=("goudy old style", 12, "bold"))
        ...
        txt_pass = Entry(self.root, textvariable=self.var_pass, font=("goudy old style", 15), bg="lightyellow", show="*").place(x=500, y=230, width=180)
        cmb_utype = ttk.Combobox(self.root, textvariable=self.var_utype, values=("ADMIN", "EMPLOYEE"), state="readonly", justify="center", font=("goudy old style", 12, "bold"))
        cmb_utype.place(x=850, y=230, width=180)
        cmb_utype.current(0)

        lbl_address = Label(self.root, text="Address", font=("goudy old style", 15), bg="white").place(x=50, y=270)
        self.txt_address = Text(self.root, font=("goudy old style", 15), bg="lightyellow")
        self.txt_address.place(x=150, y=270, width=270, height=60)
...
def search(self):
    ...
    try:
        if self.var_searchby.get() == "Select":
            ...
            if search_column:
                cur.execute(f"SELECT * FROM employee WHERE {search_column} LIKE ?", ('%' + self.var_searchtxt.get() + '%',))
                rows = cur.fetchall()
                if rows:
                    self.EmployeeTable.delete(*self.EmployeeTable.get_children())
                    for row in rows:
