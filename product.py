...
class productClass:
    def __init__(self, root):
        ...
        product_frame = Frame(self.root, bg="white", relief=RIDGE,bd=2)
        ...
        txt_category = Entry(product_frame, font=("goudy old style", 15), bg="lightyellow").place(x=150, y=60, width=200, height=27)
        ...
        txt_product_name = Entry(product_frame,textvariable=self.var_name, font=("goudy old style", 15), bg="lightyellow").place(x=150, y=160, width=200, height=27)
        lbl_price = Label(product_frame, text="Price", font=("goudy old style", 15), bg="white").place(x=30, y=210)
        txt_price = Entry(product_frame,textvariable=self.var_price, font=("goudy old style", 15), bg="lightyellow").place(x=150, y=210, width=200, height=27)
        lbl_qty = Label(product_frame, text="Quantity", font=("goudy old style", 15), bg="white").place(x=30, y=260)
        txt_qty = Entry(product_frame,textvariable=self.var_qty, font=("goudy old style", 15), bg="lightyellow").place(x=150, y=260, width=200, height=27)
        lbl_status = Label(product_frame, text="Status", font=("goudy old style", 15), bg="white").place(x=30, y=310)
        cmb_status = ttk.Combobox(product_frame, textvariable=self.var_status, values=("Available", "Not Available"), state="readonly",justify="center", font=("goudy old style", 12, "bold"))
        cmb_status.place(x=150, y=310, width=200)
        cmb_status.current(0)
        # Buttons
        btn_add = Button(product_frame, command=self.add, text="Save", font=("goudy old style", 15), bg="#2196f3", fg="white", cursor="hand2").place(x=10, y=400, width=100, height=36)
        btn_update = Button(product_frame, text="Update", command=self.update, font=("goudy old style", 15), bg="#4caf50", fg="white", cursor="hand2").place(x=120, y=400, width=100, height=36)
        btn_delete = Button(product_frame, text="Delete", command=self.delete, font=("goudy old style", 15), bg="#f44336", fg="white", cursor="hand2").place(x=230, y=400, width=100, height=36)
        btn_clear = Button(product_frame, text="Clear", command=self.clear, font=("goudy old style", 15), bg="#607d8b", fg="white", cursor="hand2").place(x=340, y=400, width=100, height=36)
    ...
    def add(self):
        ...
        try:
            ...
            if self.var_cat.get() == "Select" or self.var_cat.get() == "Empty":
                ...
                messagebox.showerror("Error", "Price must be required", parent=self.root)
                ...
                if row is not None:
                    ...
                    cur.execute(
                        "Insert into product (Supplier, Category,name, price, qty, status) VALUES (?, ?, ?, ?, ?, ?)",
                        (
                            self.var_cat.get(),
                            self.var_sup.get(),
                            self.var_name.get(),
    ...
    def update(self):
        ...
        try:
            if self.var_pid.get() == "":
                ...
                row = cur.fetchone()
                if row == None:
                    messagebox.showerror("Error", "INVALID Product , try a different one", parent=self.root)
                else:
                    cur.execute(
    ...
    #             messagebox.showerror("Error", "Select text to search", parent=self.root)
    #         else:
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get() == "Select":
                ...
                if search_column:
                    cur.execute(f"SELECT * FROM product WHERE {search_column} LIKE ?", ('%' + self.var_searchtxt.get() + '%',))
                    rows = cur.fetchall()
                    if rows:
                        self.productTable.delete(*self.productTable.get_children())
                        for row in rows:
